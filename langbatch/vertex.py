...
class VertexBatch(Batch):
    """
    VertexBatch is a class for Vertex batch processing.
    Implements the Batch class for Vertex API.
    """
    ...
    def __init__(self, file: str, model_name: str, gcp_project: str, bigquery_input_dataset: str, bigquery_output_dataset: str) -> None:
        """
        Initialize the VertexBatch class.

        Args:
            file (str): The path to the jsonl file in Vertex batch format.
    ...
    def get_status(self):
        ...
        return vertex_state_map[str(job.state.name)]
...
class VertexChatCompletionBatch(VertexBatch, ChatCompletionBatch):
    """
    VertexChatCompletionBatch is a class for Vertex chat completion batches.
    Can be used for batch processing with Gemini 1.5 Flash and Gemini 1.5 Pro models
    
    Usage:
    ```python
    batch = VertexChatCompletionBatch("path/to/file.jsonl")
    batch.start()
    ```
    """
    _url: str = "/v1/chat/completions"
    ...
    def _validate_request(self, request):
        VertexChatCompletionRequest(**request)
